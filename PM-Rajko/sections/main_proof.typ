...
#proof(title: [Proof of @thm:general-hardness-result])[
...
- Invoke Lemma @thm:partition with parameters $c=0.925$ and $gamma = 0.025$ to get subsets $A subset.eq V(G)$ and $B = V(G) without A$, such that for every $u in V(G)$ we have

$
0.9d  &<=   abs(Neighbourhood(G, u) inter A) \leq 0.95d #<eq:sizenbrA> \
0.05d  &<=   abs(Neighbourhood(G, u) inter B) \leq 0.1d #<eq:sizenbrB>
...
Map each variable $x_e$ for $e in Embedding(u) ~> Embedding(v)$ alternately to $y_(u,v)$ or $overline(y)_(u,v)$, such that the edges of the path adjacent to $Embedding(u)$ and $Embedding(v)$ are set to $y_(u,v)$.
